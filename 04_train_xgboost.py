...
snapshot_id = spark.read.format("iceberg").load('{0}.BIOMARKERS_{1}.snapshots'.format(DBNAME, USERNAME)).select("snapshot_id").tail(1)[0][0]
committed_at = spark.read.format("iceberg").load('{0}.BIOMARKERS_{1}.snapshots'.format(DBNAME, USERNAME)).select("committed_at").tail(1)[0][0].strftime('%m/%d/%Y')
parent_id = spark.read.format("iceberg").load('{0}.BIOMARKERS_{1}.snapshots'.format(DBNAME, USERNAME)).select("parent_id").tail(1)[0][0]

incReadDf = spark.read\
    .format("iceberg")\
    .option("start-snapshot-id", parent_id)\
    .option("end-snapshot-id", snapshot_id)\
    .load("{0}.BIOMARKERS_{1}".format(DBNAME, USERNAME))
...
X_train, X_test, y_train, y_test = train_test_split(df.drop("asthmatic_bronchitis", axis=1), df["asthmatic_bronchitis"], test_size=0.3)
