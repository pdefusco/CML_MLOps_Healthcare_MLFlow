...
df_from_sql = ps.read_table('{0}.BIOMARKERS_{1}'.format(DBNAME, USERNAME))
df = df_from_sql.to_pandas()

test_size = 0.3
X_train, X_test, y_train, y_test = train_test_split(df.drop("asthmatic_bronchitis", axis=1), df["asthmatic_bronchitis"], test_size=test_size)
